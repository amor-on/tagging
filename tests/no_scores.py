    ...
    with col2:
        ...
        for label in labels_info:
            ...
            if label_type == "multiselect":
                ...
                etiquetas[label_name] = selected_options
        ...
        if st.button("Guardar y ver siguiente"):
            for label_name, values in etiquetas.items():
                content_df.at[row_to_label, label_name] = ", ".join(values) if isinstance(values, list) else values

                # Guardar las selecciones actuales en session_state para la próxima fila
                st.session_state['selected_options'][label_name] = values if isinstance(values, list) else [values]
